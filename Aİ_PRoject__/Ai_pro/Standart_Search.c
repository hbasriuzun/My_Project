...
Node* First_InsertFrontier_Search_TREE(const enum METHODS method, Node *const root, State *const goal_state,COORDINATE** map)
{
    ...
    while(Number_Searched_Nodes<MAX_SEARCHED_NODE) 
    {
        ...
        for(action = (enum ACTIONS)0; action<ACTIONS_NUMBER; action += (enum ACTIONS)1)
		{
			child = Child_Node(node, action,map);
...
//______________________________________________________________________________
Node* First_GoalTest_Search_TREE(const enum METHODS method, Node *const root, State *const goal_state,COORDINATE** map)
{
    int Number_Searched_Nodes  = 0;  // The number of nodes passing goal test
	int Number_Generated_Nodes = 1;  // The number of generated nodes (The first one is the root)  
	int Number_Allocated_Nodes = 1;  // The number of nodes in memory (The first one is the root)
    ...
    Queue *frontier;
    Hash_Table *explorer_set; 
	printf("====== %d",root->state.Map_Size);
	//node->state.Map_Size = root->state.Map_Size;
	//child->state.Map_Size = root->state.Map_Size;
	// GOAL-TEST
	Number_Searched_Nodes++;
...
//______________________________________________________________________________
Node* DepthType_Search_TREE(const enum METHODS method, Node *const root, State *const goal_state, const int Max_Level,COORDINATE** map)
{
...
//______________________________________________________________________________
Node* Child_Node(Node *const parent, const enum ACTIONS action,COORDINATE** map)
{
      Node *child = NULL;
      Transition_Model trans_model;
      if( Result(&(parent->state), action, &trans_model,map) ){
          child = (Node*)malloc(sizeof(Node));
          if(child==NULL)
               Warning_Memory_Allocation(); 
           
          child->state     = trans_model.new_state;
          //child->state.Map_Size = parent->state.Map_Size;
...
Queue* Start_Frontier(Node *const root)
{
    ...
    frontier->node  = root;  
	frontier->node->state.Map_Size = root->state.Map_Size;    
    frontier->next  = NULL;
...
void Insert_Priority_Queue_A_Star(Node *const child, Queue **frontier) 
{  
    child->state.f_n = child->state.h_n + child->path_cost;    
    ...
    if(Empty(*frontier)){
	    ...
	    if(child->state.f_n < (*frontier)->node->state.f_n){ // Child has lowest cost
	        new_queue->next = *frontier;
            *frontier = new_queue; 
        }
        else{
            for(temp_queue = *frontier; temp_queue->next != NULL; temp_queue = temp_queue->next){
                if(child->state.f_n < temp_queue->next->node->state.f_n){ 
                     new_queue->next = temp_queue->next;   
                     temp_queue->next = new_queue;
                     return;
                }                                              
...
{  
    ...
    if (*frontier == NULL || f < (*frontier)->priority) {
        new_node->next = *frontier;
        *frontier = new_node;
    } else {
        Queue *current = *frontier;
        while (current->next != NULL && current->next->priority <= f) {
...
void Print_Node(const Node *const node)
{
	if(node!=NULL){
		printf("NODE(");
		Print_State_Just_Coordinate(&(node->state));
		if(node->parent){
			printf(", parent:");
			Print_State_Just_Coordinate(&(node->parent->state));
			printf(", action:");
...
{   
	...
	if(goal==FAILURE)
		...
		{
			Print_State_Just_Coordinate(&(temp->state));
			if(temp->parent!= NULL){
				printf("\n\taction(");
