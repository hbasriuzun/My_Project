...
{	
    ...
    switch(method) 
    {
        ...
        case GreedySearch:               
            goal = First_GoalTest_Search_TREE(method, &root, goal_state,map);  break; 
		case DepthFirstSearch: 	
		case DepthLimitedSearch: 
			goal = DepthType_Search_TREE(method, &root, goal_state, Max_Level,map);  break;  
        case IterativeDeepeningSearch:
            for(level=0; TRUE ;level++){
            	goal = DepthType_Search_TREE(method, &root, goal_state, level,map);
            	if(goal!=FAILURE){
        ...
        case AStarSearch:     
            goal = First_InsertFrontier_Search_TREE(method, &root, goal_state,map);  break;      
